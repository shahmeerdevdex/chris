...
import { CreditCard, Users, Activity, Clock, TrendingUp } from "lucide-react";
    ...
    try {
      ...
      subscriptionsData.forEach((subscription) => {
        ...
        if (
          ...
          new Date(subscription.subscription_end_date) > today
        ) {
          trialCount++;
        }
        // Check if the user is active (and not in trial)
          ...
          if (subscription.subscription_start_date) {
            ...
            while (currentDate <= endDate) {
              ...
              const trialEndDate = subscription.subscription_end_date
                ? new Date(subscription.subscription_end_date)
                : null;
            ...
            const startMonthKey = `${startDate.getFullYear()}-${
            ...
            if (
              ...
              const endMonthKey = `${endDate.getFullYear()}-${
                endDate.getMonth() + 1
              }`;
      ...
      const conversionRate =
        totalUsersEver > 0 ? activeCount / totalUsersEver : 0;

      // Calculate churn rate based on ended subscriptions divided by total
      const endedSubscriptions = subscriptionsData.filter(
        (s) => s.subscription_end_date && s.status !== "active"
      ).length;
      const churnRate =
        totalUsersEver > 0 ? endedSubscriptions / totalUsersEver : 0;
      ...
      const monthlyRevenueArray = Array.from(monthlyRevenue.values()).sort(
        (a, b) =>
          a.year === b.year ? a.monthOrder - b.monthOrder : a.year - b.year
      );

      // Store formatted monthly revenue data in localStorage for the chart component
      localStorage.setItem(
        "monthlyRevenueData",
        JSON.stringify(monthlyRevenueArray)
      );
    } catch (error) {
      ...
      <main className="flex-1 overflow-auto">
        <div className="container py-6">
          <div className="mb-8">
            ...
            <p
              className="text-muted-foreground animate-slide-down"
